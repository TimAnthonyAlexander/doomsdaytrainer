 ...
 *
 * Structure comes last, and only the first time. Put first, it is the route the
  ...
  switch (phase.kind) {
    ...
    case 'batch-recall':
      return phase.batch + 1 < batches
        ...
        : { kind: 'all-ten' };
    case 'all-ten':
      return options.structureSeen ? { kind: 'done' } : { kind: 'structure' };
    default:
      return { kind: 'done' };
  }
}
...
export function phaseSequence(options: FlowOptions): LearnPhase[] {
  ...
  const limit = (options.batches ?? BATCH_COUNT) * 2 + 3;
